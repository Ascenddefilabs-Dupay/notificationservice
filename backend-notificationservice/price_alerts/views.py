        ...
        try:
            ...
            for currency in currencies:
                ...
                if abs(price_change) >= currency.price_change_threshold:
                    ...
                    if currency.last_notified_date == today:
                        logger.info(f"Notification already sent today for {currency.symbol}. Skipping.")
                        continue  # Skip sending notification again for today
                    ...
                    # Update the last_notified_date to today
                    currency.last_notified_date = today
                    currency.save()
            ...
            if notifications_content:
                ...
                return Response({
                    "message": "No price changes exceeded the thresholds or notifications already sent today.",
                }, status=status.HTTP_200_OK)

        except requests.HTTPError as http_err:
            logger.error(f"HTTP error occurred: {str(http_err)}")
            return Response({"error": "Failed to fetch data from CoinGecko."}, status=status.HTTP_502_BAD_GATEWAY)
        except Exception as e:
